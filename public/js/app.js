! function(t) {
  ...
  jQuery(function($) {
          ...
          console.log($('#pickfiles')[0],"dsfdsfdsf")

      
        var uploader = new plupload.Uploader({
            multi_selection: false,
            runtimes: 'html5,flash,silverlight,html4',
            browse_button:$('#pickfiles')[0], // you can pass an id...
            container:$('.pro-sec.pro-sec-1')[0], // ... or DOM Element itself
            url: 'http://localhost/keeplaralatest/public/profile',
           /* flash_swf_url: 'https://csnaps.org/wp-content/themes/csnaps/js/plupload/Moxie.swf',
            silverlight_xap_url: 'https://csnaps.org/wp-content/themes/csnaps/js/plupload/Moxie.xap',*/

            filters: {
                max_file_size: '10mb',
                min_image_size: [250, 150],
                mime_types: [{
                    title: "Image files",
            ...
            init: {
                ...
                FilesAdded: function(up, files) {
                    plupload.each(files, function(file) {
                        $('input#loadingblockP').val("0%");
                        ProfileModal.find('#pickfiles').html('<i class="fa fa-spinner"></i>');
                    });
                    uploader.start();
                },
                UploadProgress: function(up, file) {
                    $('input#loadingblockP').val(file.percent + "%");
                },
                Error: function(up, err) {
                    var cont = res_modal.find('#f_response');
                    var error_block = $('<p class="error">Error #' + err.code + ": " + err.message + "</p>").prependTo(cont);
                    setTimeout(function() {
                        error_block.remove();
                    }, 10000);
                },
                FileUploaded: function(up, fileobj, response) {
                    var file = response.response;
                    var data = $.parseJSON(file);
                    ProfileModal.find('#pickfiles').html('Choose file');

                    if (data.status) {
                        ProfileImage = data.image;
                    } else {
                        var cont = ProfileModal.find('#f_response');
                        var error_block = $('<p class="error">' + data.message + "</p>").prependTo(cont);
                        setTimeout(function() {
                            error_block.remove();
                        }, 10000)
                    }
